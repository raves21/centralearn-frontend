...
export default function IdentificationBlockDisplay({
  ...
  return (
    <div className="flex flex-col gap-7 p-6 rounded-md bg-white">
      ...
      />
      <div className="flex flex-col gap-5 p-6 rounded-md border border-gray-200 w-fit">
        ...
        <div className="flex items-center flex-wrap gap-4 border border-gray-200 w-min bg-green-100 p-1 rounded-md">
          {assessmentMaterial.material.acceptedAnswers.map(
            (acceptedAnswer, i) => (
              <p key={i} className="p-2 rounded-md w-fit whitespace-nowrap">
                {acceptedAnswer}
              </p>
            ),
          )}
